...
struct ptl_cq *ptl_cq_pool_get(struct ptl_cq_pool *ptl_cq_pool, int nr_cqes,
  ...
  /* Sanity checks */
  if (!ptl_cq_pool) {
    PTL_WARN("NULL pool pointer?");
    return ERR_PTR(-EINVAL);
  }

  if (nr_cqes <= 0) {
    PTL_WARN("invalid nr_cqes %d", nr_cqes);
    return ERR_PTR(-EINVAL);
  }

  if (nr_cqes > PTL_CQ_POOL_MAX_SIZE) {
    PTL_WARN("nr_cqes: %d larger than maximum: %d", nr_cqes,
  ...
  cpumask_t mask;
  cpumask_clear(&mask);
  int node = 0; // TODO: replace by NIC's numa node
  int cpu = cpumask_first_and(cpumask_of_node(node), cpu_online_mask);
  if (cpu >= nr_cpu_ids)
    cpu = cpumask_first(cpu_online_mask); // fallback
  cpumask_set_cpu(cpu, &mask);
  ptl_cq = ptl_cq_create(ptl_cq_pool, nr_cqes, pte, poll_ctx, &mask);
  if (IS_ERR(ptl_cq)) {
    PTL_WARN("ptl_cq_pool_get: failed to create new CQ");
    return ptl_cq;
  }
