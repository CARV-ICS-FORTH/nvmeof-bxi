...
//  device_attr->node_guid = 0x0002c90300fed670;
	...
	attrs->max_pd = 32764;
	/*
	 * Since I am an initiator I am not allowed me the initiator to perform rdma read or writes to the target.
	 * Period. Set it to 0 for this purpose.
	 */
...
struct ptl_bxiv3_device *ptl_bxiv3_dev_create(u32 iface_id)
{
	...
	if (PTL_OK != rc) {
		PTL_FATAL("Failed to get physical id of #iface: %d reason: %d",
		          iface_id, rc);
		ret = -EIO;
		goto clean_up;
	}
	...
	rc = PtlEQAsyncIntrAlloc(bxiv3_dev->nicia_handle, &cpu_mask, &bxiv3_dev->intr_index);
	if (PTL_OK != rc) {
		PTL_FATAL
		("Failed to allocate interrupt index for BXIv3 with error code: %d",
		 rc);
		goto clean_up;
	}
	bxiv3_dev->conn_mgmt_eq = ptl_cq_create(NULL, bxiv3_dev, PTL_CP_SERVER_CQ_ENTRIES, PTL_CP_SERVER_PTE, IB_POLL_SOFTIRQ);
	...
	for (u32 i = 0; i < PTL_BXIV3_DEVICE_CONNECTION_NUM_RECV_BUFFERS; i++) {
		PTL_DEBUG("Recv buffer size: %lu allocation size: %lu", sizeof(*recv_buffer), RDMA_PTL_MSG_BUFFER_SIZE);
	...
	PTL_INFO
	("Initialized interface: %d with nid: %d or (>>7:%d) and pid: %d "
	 "successfully", iface_id, bxiv3_dev->proc_id.phys.nid,
...
ptl_pt_index_t ptl_bxiv3_dev_alloc_pte(struct ptl_bxiv3_device *bxiv3_dev)
{
	...
	bit =
	        find_first_zero_bit(bxiv3_dev->pte_table,
	                            PTL_BXIV3_DEVICE_MAX_PTES);
...
int ptl_bxiv3_dev_destroy(struct ptl_bxiv3_device *bxiv3_dev)
{
	...
	rc = PtlEQAsyncIntrFree(bxiv3_dev->nicia_handle,
	                        bxiv3_dev->intr_index);
	if (PTL_OK != rc) {
		PTL_WARN
		("Failed to destroy interrupt vector with error code: %d. OK, life "
		 "goes on", rc);
	}
	/* Finalize the NI handle */
	rc = PtlNIFini(bxiv3_dev->nicia_handle);
	if (PTL_OK != rc) {
		PTL_WARN("Failed to shut down nicia handle of iface id: %d with code: %d", bxiv3_dev->iface_id, rc);
		goto error;
	}
