...
struct ptl_cm_id *ptl_cm_id_create(struct net *net,
  ...
  /*Staff for the bottom layer of the driver to work */
...
int ptl_cm_id_resolve_addr(struct ptl_cm_id *ptl_cm_id,
  ...
  case AF_INET: {
    ...
    ptl_cm_id->nid = ((unsigned char *)&addr_in->sin_addr.s_addr)[3];

    /* Port number (convert from network to host byte order) */
    ptl_cm_id->pid = ntohs(addr_in->sin_port);
    break;
  }
  case AF_INET6: {
    struct sockaddr_in6 *addr_in6 = (struct sockaddr_in6 *)dst_addr;

    /* Last byte of IPv6 address */
    ptl_cm_id->nid = addr_in6->sin6_addr.s6_addr[15];

    /* Port number (convert from network to host byte order) */
    ptl_cm_id->pid = ntohs(addr_in6->sin6_port);
    break;
  }
  default:
    return -EAFNOSUPPORT;
  }
  ptl_cm_id->nid = ptl_cm_id->nid << 7;
  PTL_DEBUG("Resolved Target's nid: %d pid: %d", ptl_cm_id->nid,
            ptl_cm_id->pid);
  PTL_WARN("Statically assign ptl_cm_id to BXIv3-0 XXX TODO XXX: spread it "
           "dynamically");
