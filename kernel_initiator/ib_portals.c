...
void ib_portals_cq_pool_put(struct ib_cq *cq, int cqe) {
  ...
  PTL_DEBUG("Returned PTL_CQ (in PTE: %d) to the pool successfully ...",
            ptl_cq->pte);
}
...
dma_addr_t ib_portals_dma_map_single(struct ib_device *ibdev, void *cpu_addr,
  ...
  PTL_DEBUG("CPU Virtual Addr: %p <---> DMA Addr: %pad (size: %zu)", cpu_addr,
            &dma_addr, size);
  return dma_addr;
}
...
int ib_portals_dma_unmap_sg(struct ib_device *ibdev, struct scatterlist *sgl,
  ...
  for_each_sg(sgl, s, nents, i) {
    PTL_DEBUG("DMA_UNMAP_sg: SG[%d]: virt_addr=%p, iova=%llx, length=%u", i,
              sg_virt(s), sg_dma_address(s), s->length);
  }
...
#ifndef PTL_RELEASE
  dma_addr_t prev_end_addr = 0;
  bool first = true;

  /* Print and check scatter-gather list after mapping */
  for_each_sg(sgl, s, ret, i) {
    dma_addr_t curr_addr = sg_dma_address(s);
    unsigned int curr_len = sg_dma_len(s);

    PTL_DEBUG("  POST-MAP-SG[%d out of %d]: dma_addr=%pad, "
              "dma_length=%u original nents were: %d",
              i, ret, &curr_addr, curr_len, nents);

    /* Check for contiguous address space */
    if (curr_addr == 0) {
      PTL_FATAL("SG[%d] has zero DMA address", i);
      return -EINVAL;
    }

    if (curr_len == 0) {
      PTL_FATAL("SG[%d] has zero length", i);
      return -EINVAL;
    }

    if (!first && curr_addr != prev_end_addr) {
      PTL_FATAL("DMA mapping is not contiguous: SG[%d] addr=%pad expected=%pad",
                i, &curr_addr, &prev_end_addr);
      return -EINVAL;
    }

    prev_end_addr = curr_addr + curr_len;
    first = false;
  }
#endif
...
static void ib_portals_send_nvmeof_cmd(struct ptl_qp *ptl_qp,
  ...
  if (IB_SEND_SIGNALED == send_wr->send_flags) {
    PTL_DEBUG(
        "Creating notification event when delivery of this nvme_cmd completes");
  ...
  ptl_uuid_set_initiator_qp_num(&msg.hdr_data,
  ...
  PTL_DEBUG("Send NVMeOF cmd: Target qp num: %d initiator qp num: %d cq_id: %d",
            ptl_qp->ptl_id->target_qp_num, ptl_qp->ptl_id->initiator_qp_num,
            ptl_qp->ptl_id->remote_cq_id);

  rc = PtlMsgPutOnce(ptl_qp->ptl_id->bxiv3_dev->nicia_handle, &md, &msg);
  if (PTL_OK != rc) {
    PTL_FATAL("Failed to send NVMeOF command with error: %s",
...
int ib_portals_post_send(struct ib_qp *qp, struct ib_send_wr *wr,
  ...
  for (curr = wr; curr != NULL; curr = curr->next) {
    ...
    switch (curr->opcode) {
    case IB_WR_REG_MR:
      PTL_DEBUG("OPCODE: IB_WR_REG_MR treat is a no-op, XXX TODO XXX use it "
                "when matching is enabled in the future");
      break;
...
int ib_portals_post_recv(struct ib_qp *qp, struct ib_recv_wr *recv_wr,
  ...
  for (wr = recv_wr; wr != NULL; wr = wr->next) {
    ...
    PTL_DEBUG("WR[%d]: addr=%llx, wr_id=%llu, num_sge=%d for QPN: %d", i,
              recv_wr->sg_list[0].addr, recv_wr->wr_id, recv_wr->num_sge,
              ptl_qp->qpn);
    ...
    for (int j = 0; j < recv_wr->num_sge; j++) {
      ...
      recv_op_meta_idx =
          (recv_wr->sg_list[j].addr - ptl_qp->ptl_id->nvme_cpl_start) /
      ...
      //    if (PTL_OK != rc) {
      //      PTL_FATAL("LEAppend failed. Reason: %s", PtlToStr(rc,
      //      PTL_STR_ERROR));
      // }
...
/**
 * gesalous:
 * Mellanox NICs take a set of possibly non-contiguous DMA addresses translated
 * by the kernel in a previous step and map them into a contiguous address space
 * within the NIC. BXIv3 lacks a similar hardware mechanism. Instead, it
 * achieves the same result using the IOMMU, where the kernel maintains a
 * separate page table specifically for the NIC.
 *
 * In the preceding translation step, the kernel constructs a contiguous IOVA
 * (I/O Virtual Address) mapping in the IOMMU for the non-contiguous physical
 * addresses, achieving the same behavior. While multiple IOVAs may be returned
 * in certain cases, they must remain virtually contiguous.
 *
 * This multi-IOVA scenario occurs because the kernel respects the maximum
 * segment size supported by the peripheral (the BXIv3 NIC) for DMA transfers.
 * If this size is exceptionally small (e.g., 128 bytes), the kernel
 * intentionally maps the transfer to a sequence of contiguous IOVAs to prevent
 * starving the PCIe link. This function verifies this condition to ensure that
 * the resulting IOVAs are indeed contiguous.
 */
...
int ib_portals_map_mr_sg(struct ib_mr *mr, struct scatterlist *sg, int sg_nents,
  ...
  if (!mr || !sg || sg_nents <= 0) {
    PTL_FATAL("Invalid parameters: mr=%p, sg=%p, sg_nents=%d", mr, sg,
              sg_nents);
    return -EINVAL;
  }
  ...
  if (sg_dma_address(&sg[0])) {
    ...
    mapped_nents =
        ib_portals_dma_map_sg(mr->device, sg, sg_nents, DMA_BIDIRECTIONAL);
    if (mapped_nents <= 0) {
  ...
  for_each_sg(sg, s, sg_nents, i) {
    ...
    PTL_DEBUG("%s  [%d] dma_addr: 0x%llx, dma_len: %u for device: %s",
              skipped ? "Already mapped" : "Mapped", i, dma_addr, dma_len,
              dev_name(PtlGetDriverDev(ptl_mr->bxi3_device->nicia_handle)));
  }
  ...
  PTL_DEBUG("=== MR populated: iova=0x%llx, length=%llu === sg_nents are: %d "
            "mapped_nents: %d",
...
int ib_portals_enable_rma_ops(struct ib_qp *qp, struct ib_cq *cq) {
  ...
  rc =
      PtlLEAppend(ptl_qp->ptl_id->bxiv3_dev->nicia_handle, ptl_qp->recv_cq->pte,
                  &ptl_qp->rma_le, PTL_PRIORITY_LIST, ptl_qp, &ptl_qp->rma_leh);
  if (PTL_OK != rc) {
    PTL_FATAL("Failed to expose address space for rma + nvme_cpl operations. "
              "Reason: %s",
              PtlToStr(rc, PTL_STR_ERROR));
  }
  ...
  PTL_DEBUG("Enable RMA ops for PTE: %d for QPN: %d", ptl_qp->recv_cq->pte,
            ptl_qp->qpn);
  return 1;
}
