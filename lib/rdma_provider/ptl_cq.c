...
struct ptl_cq *ptl_cq_create(void *cq_context)
{
	...
	ptl_cq->object_type = PTL_CQ;
	ptl_cq->cq_id = __sync_fetch_and_add(&next_cq_id, 1);

	SPDK_PTL_DEBUG("PtlCQ: Initialized completion queue for cq_id: %d", ptl_cq->cq_id);
	ptl_cq->fake_ibv_cq.context = ptl_cnxt_get_ibv_context(ptl_cnxt_get());

	ret = PtlEQAlloc(ptl_cnxt_get_ni_handle(ptl_cnxt_get()), PTL_CQ_SIZE, &ptl_cq->eq_handle);
	if (ret != PTL_OK) {
		SPDK_PTL_FATAL("PtlEQAlloc failed with error code %d", ret);
	}
	ptl_cq->pte = -1;
