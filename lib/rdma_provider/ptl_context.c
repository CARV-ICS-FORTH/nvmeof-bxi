...
const char *help_message =
    "ROLE=<target,initiator>(mandatory) PORTALS_NID=<nid number> (optional) "
    "PORTALS_PID=<pid number> (mandatory only for target)";
static struct ptl_context ptl_context;
typedef struct ptl_context_op_meta *(*process_event)(ptl_event_t event,
                                                     struct ibv_wc *wc,
...
static inline void
ptl_cnxt_destroy_op_meta(struct ptl_context_op_meta *op_meta) {
  free(op_meta);
}
...
static void ptl_cnxt_keep_event(struct ptl_cq *ptl_cq, struct ibv_wc *wc,
  ...
  if (ptl_cq->pending_completions == NULL) {
    SPDK_PTL_FATAL("pending_completions is NULL for ptl_cq id: %d in use: %d",
                   ptl_cq->cq_id, ptl_cq->is_in_use);
  }
  ptl_wc->wc = *wc;
...
static struct ptl_context_op_meta *ptl_cnxt_process_get(ptl_event_t event,
                                                        struct ibv_wc *wc,
                                                        struct ptl_cq *ptl_cq) {
  SPDK_PTL_DEBUG(
      "NVMe: Someone performed an RDMA READ from me, ignore Portals internal");
  return NULL;
}

static struct ptl_context_op_meta *
ptl_cnxt_process_get_overflow(ptl_event_t event, struct ibv_wc *wc,
                              struct ptl_cq *ptl_cq) {
...
static struct ptl_context_op_meta *ptl_cnxt_process_put(ptl_event_t event,
                                                        struct ibv_wc *wc,
                                                        struct ptl_cq *ptl_cq) {
  ...
  if (NULL == event.user_ptr) {
    SPDK_PTL_DEBUG(
        "NVMe: RECV operation with null context received an RDMA_WRITE");
    return NULL;
  }
  ...
  if (event.start != recv_meta->recv_op.io_vector[0].iov_base) {
    SPDK_PTL_FATAL("Corrupted receive event.start: %p event.legnth: %lu "
                   "iovector[0] = %p iovector size[0] = %lu pte: %d",
                   event.start, event.rlength,
                   recv_meta->recv_op.io_vector[0].iov_base,
                   recv_meta->recv_op.io_vector[0].iov_len, event.pt_index);
  }

  recv_meta->recv_op.initiator_qp_num =
      ptl_uuid_get_initiator_qp_num(&event.hdr_data);
  recv_meta->recv_op.target_qp_num =
      ptl_uuid_get_target_qp_num(&event.hdr_data);
  ...
  if (recv_meta->recv_op.initiator_qp_num == 0 ||
      recv_meta->recv_op.target_qp_num == 0) {
    SPDK_PTL_DEBUG(
        "Staff from header data: initiator qp num %d target qp num: %d",
        ptl_uuid_get_initiator_qp_num(&event.hdr_data),
        ptl_uuid_get_target_qp_num(&event.hdr_data));
...
static struct ptl_context_op_meta *
ptl_cnxt_process_put_overflow(ptl_event_t event, struct ibv_wc *wc,
                              struct ptl_cq *ptl_cq) {
...
static struct ptl_context_op_meta *
ptl_cnxt_process_atomic(ptl_event_t event, struct ibv_wc *wc,
                        struct ptl_cq *ptl_cq) {
  SPDK_PTL_FATAL("UNIMPLEMENTED");
  return NULL;
}
...
static struct ptl_context_op_meta *
ptl_cnxt_process_atomic_overflow(ptl_event_t event, struct ibv_wc *wc,
                                 struct ptl_cq *ptl_cq) {
...
static struct ptl_context_op_meta *
ptl_cnxt_process_fetch_atomic(ptl_event_t event, struct ibv_wc *wc,
                              struct ptl_cq *ptl_cq) {
...
static struct ptl_context_op_meta *
ptl_cnxt_process_fetch_atomic_overflow(ptl_event_t event, struct ibv_wc *wc,
                                       struct ptl_cq *ptl_cq) {
 ...
 */
static struct ptl_context_op_meta *
ptl_cnxt_process_reply(ptl_event_t event, struct ibv_wc *wc,
                       struct ptl_cq *ptl_cq) {
  ...
  if (rdma_read_meta == NULL) {
    SPDK_PTL_DEBUG(
        "Caution RDMA read without a context app does not want a signal ok.");
    return NULL;
  }
  ...
  if (++rdma_read_meta->rdma_read_op.parts_acked <
  ...
  SPDK_PTL_DEBUG("NVMe: RDMA read done (PTL_EVENT_REPLY). Number of bytes "
                 "received: %lu. Filling wc with code %d qp_num: %d",
                 event.rlength, event.ni_fail_type,
                 rdma_read_meta->send_op.qp_num);

  return rdma_read_meta;
}

static struct ptl_context_op_meta *
ptl_cnxt_process_send(ptl_event_t event, struct ibv_wc *wc,
                      struct ptl_cq *ptl_cq) {
  return NULL;
}
 ...
 * for a PtlPut operation that explicitly requested an acknowledgment. In
 * Portals 4, PTL_EVENT_ACK is an initiator-side event that indicates the target
 * has matched the incoming PUT against its match list (ME/LE) and completed the
 * delivery to the target memory according to Portals 4 semantics.
 *
 * Notes:
 * - Applies to PtlPut regardless of whether the initiator’s usage corresponds
 * to a send or an (RMA) PUT. In both cases, the ACK confirms data are in the
 * memory of the Target.
 * - This is stronger than local send completion (PTL_EVENT_SEND). PTL_EVENT_ACK
 * means the target has received, matched, and placed the data into its memory.
 * - You will not receive PTL_EVENT_ACK for PtlGet; GET completion at the
 * initiator is indicated by PTL_EVENT_REPLY instead.
 *
 * On success (event.ni_fail_type == PTL_NI_OK), this function populates the
 * provided ibv_wc with a successful SEND completion semantics for the original
 * PUT, using the metadata carried in event.user_ptr, and returns the associated
 * ptl_context_op_meta. On failure, it reports the Portals NI failure code.
 */
static struct ptl_context_op_meta *ptl_cnxt_process_ack(ptl_event_t event,
                                                        struct ibv_wc *wc,
                                                        struct ptl_cq *ptl_cq) {
  struct ptl_context_op_meta *send_meta;

  if (NULL == event.user_ptr) {
    SPDK_PTL_FATAL("NVMe: PTL_EVENT_ACK without context? Not possible");
  }
  send_meta = event.user_ptr;

  if (send_meta->obj_type != PTL_RDMA_WRITE_OP) {
    SPDK_PTL_FATAL("Corrupted object type: %d this is not a PTL_SEND_OP or "
                   "PTL_RDMA_WRITE_OP",
                   send_meta->obj_type);
  }
  ...
  if (event.ni_fail_type != PTL_NI_OK) {
    if (send_meta->obj_type == PTL_RDMA_WRITE_OP) {
      SPDK_PTL_FATAL("Operation of type PTL_RDMA_WRITE_OP failed with reason: "
                     "%s faulting address: 0x%016lx nvme_cid: %u",
                     PtlToStr(event.ni_fail_type, PTL_STR_FAIL_TYPE),
                     (unsigned long)send_meta->rdma_write_op.addr,
                     send_meta->nvme_cid);
    }
    SPDK_PTL_FATAL("Operation of type PTL_SEND_OP failed with reason: %s "
                   "faulting address: 0x%016lx",
                   PtlToStr(event.ni_fail_type, PTL_STR_FAIL_TYPE),
                   (unsigned long)send_meta->send_op.addr);
  }

  if (++send_meta->rdma_write_op.parts_acked <
      send_meta->rdma_write_op.total_parts) {
    return NULL;
  }
  ...
  SPDK_PTL_DEBUG(
      "Got ack for send for wr_id: %lu mismatch of queues? %s send ack "
      "is for queue: %d current ptl_cq is: %d",
      send_meta->wr_id,
      ptl_cq_get_id(ptl_cq) != send_meta->cq_id ? "YES" : "NO",
...
static struct ptl_context_op_meta *
ptl_cnxt_process_bt_disabled(ptl_event_t event, struct ibv_wc *wc,
                             struct ptl_cq *ptl_cq) {
...
static struct ptl_context_op_meta *
ptl_cnxt_process_auto_unlink(ptl_event_t event, struct ibv_wc *wc,
                             struct ptl_cq *ptl_cq) {
  struct ptl_context_op_meta *recv_meta;
  ...
  if (false == recv_meta->recv_op.receive_done) {
    SPDK_PTL_FATAL("AUTO_UNLINK without a prior receive for "
                   ...
                   ptl_cq_get_id(ptl_cq), event.ni_fail_type);
  }
  ...
  wc->qp_num = is_target ? recv_meta->recv_op.target_qp_num
                         : recv_meta->recv_op.initiator_qp_num;

  SPDK_PTL_DEBUG(
      "NVMe-cmd-recv: RECV (PtlPut+AUTO_UNLINK) operation is "
      ...
      is_target ? "YES" : "NO", recv_meta->recv_op.bytes_received, wc->qp_num,
      (uint64_t)recv_meta->recv_op.io_vector[0].iov_base);

  if (wc->qp_num == 0) {
    SPDK_PTL_FATAL("Nida does not assign 0 fake qp numbers is_target? %s pair "
  ...
  SPDK_PTL_DEBUG(
      "Got ack for recv for wr_id: %lu mismatch of queues? %s send ack "
      "is for queue: %d current ptl_cq is: %d",
      recv_meta->wr_id,
      ptl_cq_get_id(ptl_cq) != recv_meta->cq_id ? "YES" : "NO",
      recv_meta->cq_id, ptl_cq_get_id(ptl_cq));
  return recv_meta;
}

static struct ptl_context_op_meta *
ptl_cnxt_process_auto_free(ptl_event_t event, struct ibv_wc *wc,
                           struct ptl_cq *ptl_cq) {
...
static struct ptl_context_op_meta *
ptl_cnxt_process_search(ptl_event_t event, struct ibv_wc *wc,
                        struct ptl_cq *ptl_cq) {
...
static struct ptl_context_op_meta *
ptl_cnxt_process_link(ptl_event_t event, struct ibv_wc *wc,
                      struct ptl_cq *ptl_cq) {
...
static int ptl_print_event(struct ptl_context_op_meta *op_meta, bool is_late) {
  ...
  if (op_meta->obj_type == PTL_SEND_OP &&
      op_meta->send_op.crc_checksum !=
          ptl_cnxt_calculate_crc64(op_meta->send_op.addr,
  ...
  if (op_meta->obj_type == PTL_SEND_OP) {
    SPDK_PTL_INFO("SEND Event Print: %d",
                  op_meta->send_op.length == 64
                      ? ptl_print_nvme_cmd(op_meta->send_op.addr,
                                           is_late ? prefix_cmd_send_late
                                                   : prefix_cmd_send,
                                           op_meta->cq_id)
                      : ptl_print_nvme_cpl(op_meta->send_op.addr,
                                           is_late ? prefix_cpl_send_late
                                                   : prefix_cpl_send,
                                           op_meta->cq_id));
  } else if (op_meta->obj_type == PTL_RECV_OP) {
    SPDK_PTL_INFO("RECV Event Print: %d",
                  op_meta->recv_op.bytes_received == 64
                      ? ptl_print_nvme_cmd(
                            op_meta->recv_op.io_vector[0].iov_base,
                            is_late ? prefix_cmd_recv_late : prefix_cmd_recv,
                            op_meta->cq_id)
                      : ptl_print_nvme_cpl(
                            op_meta->recv_op.io_vector[0].iov_base,
                            is_late ? prefix_cpl_recv_late : prefix_cpl_recv,
                            op_meta->cq_id));
...
static int ptl_cnxt_poll_cq(struct ibv_cq *ibv_cq, int num_entries,
  ...
  while (events_processed < num_entries) {
    ...
    SPDK_PTL_DEBUG("PtlCQ: Delivered late event for ptl_cq id: %d wr_id = %lu",
                   ptl_cq->cq_id, ptl_late_wc->wc.wr_id);
    wc[events_processed++] = ptl_late_wc->wc;
    SPDK_PTL_DEBUG("Late event: %d",
                   ptl_print_event(ptl_late_wc->op_meta, true));
  ...
  while (events_processed < num_entries) {
    ...
    if (ret == PTL_OK) {
      ...
      if (ptl_cq->cq_id != op_meta->cq_id) {
        ...
        ptl_cnxt_keep_event(&ptl_cq_array[op_meta->cq_id],
                            &wc[events_processed], op_meta);
        continue;
      }

      SPDK_PTL_DEBUG("PtlCQ Delivered on-time event: %d",
...
static void ptl_cnxt_init_rma_pte(struct ptl_context *ptl_cnxt) {
...
#else
  /**
   * XXX TODO XXX ptl_context.is_target is directly inferred from the env
   * variable. On the other hand, is_target volatile variable is inferred when
   * either rdma_listen or rdma_connect is called. One of them should go.
   */
  ...
  if (rc) {
    SPDK_PTL_FATAL("Event queue creation failed with error: %s",
  ...
  if (PTL_OK != rc) {
    SPDK_PTL_FATAL("PT alloc for PTE: %d failed with error: %s",
                   ptl_cnxt->portals_idx_rma, PtlToStr(rc, PTL_STR_ERROR));
  }
  rc = PtlPTEnable(ptl_cnxt->ni_handle, ptl_cnxt->rma_pte_handle);
  if (PTL_OK != rc) {
    SPDK_PTL_FATAL("Failed to enable PTE: %d with error: %s",
                   ptl_cnxt->portals_idx_rma, PtlToStr(rc, PTL_STR_ERROR));
  }
#endif
}
...
static void ptl_cnxt_dev_print_ni_limits(const struct ptl_ni_limits *limits) {
  ...
  SPDK_PTL_DEBUG("  max_unexpected_headers= %d",
  ...
  SPDK_PTL_DEBUG("  max_fetch_atomic_size = %lu",
...
struct ptl_context *ptl_cnxt_get(void) {
  ...
  role = getenv("ROLE");
  if (NULL == role) {
    SPDK_PTL_FATAL("Sorry you need to set the role of this process (target, "
                   "initiator). %s",
                   help_message);
  }

  if (strcmp(role, "target") && strcmp(role, "initiator")) {
    SPDK_PTL_FATAL("ROLE should be either targer or initiator. %s",
                   help_message);
  }
  ...
  if (NULL == ptl_pid && 0 == strcmp(role, "target")) {
    SPDK_PTL_FATAL(
        "Sorry you need to set SERVER_PID env variable for the target case. %s",
        help_message);
  }

  if (NULL == ptl_pid && 0 == strcmp(role, "initiator")) {
    SPDK_PTL_WARN(
        "The system will assign automatically a PID for the initiator role");
  }

  ptl_nid = getenv("PORTALS_NID");
  if (NULL == ptl_pid && 0 == strcmp(role, "target")) {
    SPDK_PTL_FATAL(
        "Sorry you need to set SERVER_NID env variable for the target case. %s",
        help_message);
  }

  if (NULL == ptl_pid && 0 == strcmp(role, "initiator")) {
    SPDK_PTL_WARN(
        "The system will assign automatically an NID for the initiator role");
  }
  ...
  // } else {
  // 	SPDK_PTL_FATAL("Total data ordering is not supported by this
  // implementation. Cannot support NVMe-OF properties");
  // }
  ...
  ptl_context.pte_table =
      calloc(ptl_context.pte_table_size, sizeof(*ptl_context.pte_table));
  ...
  SPDK_PTL_DEBUG("SUCCESSFULLY create and initialized PORTALS context with "
                 "matcing enabled with role: %s",
                 role);
...
#if PTL_USE_MATCHING
  SPDK_PTL_FATAL(
      "You should not call this function in the PTL_USE_MATCHING case");
#else
...
int ptl_cnxt_allocate_pte(struct ptl_context *cnxt) {
  ...
  pthread_mutex_lock(&cnxt->pte_table_lock);
#if PTL_USE_MATCHING
  /*In the case of matching we use a single PTE PTL_PT_INDEX for the NVMe-cmd,
   * NVMe-cpl, and RMA operations*/
